...
class HourglassModel():
        ...
        def _graph_hourglass(self, inputs):
                ...
                with tf.name_scope('model'):
                        with tf.name_scope('preprocessing'):
                                ...
                                r1 = self._residual_module(conv1, numOut = 128, name = 'r1')
                                ...
                                if self.tiny:
                                        r3 = self._residual_module(pool1, numOut=self.nFeat, name='r3')
                                else:
                                        r2 = self._residual_module(pool1, numOut= int(self.nFeat/2), name = 'r2')
                                        r3 = self._residual_module(r2, numOut= self.nFeat, name = 'r3')
                        # Storage Table
                        hg = [None] * self.nStack
                        ll = [None] * self.nStack
        ...
        def _conv_blocks(self, inputs, numOut, name = 'conv_block'):
        ...
        def _res_connect(self, inputs, numOut, name = 'skip_layer'):
                """ residual connection
                Args:
                        inputs        : Input Tensor
                        numOut        : Desired output number of channel
        ...
        def _residual_module(self, inputs, numOut, name = '_residual_module'):
                """ Residual Unit
                Args:
                        inputs        : Input Tensor
                ...
                with tf.name_scope(name):
                        #################################################################################
                        # Exercise 1: Please complete the residual module.                                                          #
                        # Hint: Using the functions _conv_blocks and _res_connect above might be easier #
                        #                 We have already defined the I/O for you and left some clues                         #
                        #################################################################################
                        conv = self._conv_blocks(inputs, numOut)
                        res  = self._res_connect(inputs, numOut)
                        return conv + res
        ...
        def _hourglass(self, inputs, n, numOut, name = 'hourglass'):
                ...
                """
                #########################################################################
                # Exercise 2: we have already implemented a level 1 hourglass network.  #
                # Now you are asked to implement a level 4 hourglass network.                        #
                # Hint: Only changing one of lines below can achieve the goal.          #
                #########################################################################

                with tf.name_scope(name):
                        # Upper Branch
                        up_1 = self._residual_module(inputs, numOut, name = 'up_1')
                        # Lower Branch
                        low_ = tf.contrib.layers.max_pool2d(inputs, [2,2], [2,2], padding='VALID')
                        low_1= self._residual_module(low_, numOut, name = 'low_1')
                        low_2= self._residual_module(low_1, numOut, name = 'low_2') if n == 0 else self._hourglass(low_1, n-1, numOut, name = 'low_2')
                        low_3= self._residual_module(low_2, numOut, name = 'low_3')
